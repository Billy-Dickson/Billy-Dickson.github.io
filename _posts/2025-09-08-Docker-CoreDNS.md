...
The only downside for me, was the lack of encrypted DNS , yes it does have DOH (DOT DNS over TLS) that my previous pfsense appliance had built in by default, yes the Unifi Gateway Ultra does have DOH (DNS over HTTPS), but if I decide at some if I decide to change my router to another brand, its another thing that I wouldn't have to think about, that and I do like leaning new things, so I did a bit of research and decided to spin up a docker container running CoreDNS to do name resolution for my bit of the home network, I created my own VLAN so that I can mess about and not affect the rest of the house, my better looking half works from home and needs things to just work, and she didn't need a kaos monkey (me) breaking things.
